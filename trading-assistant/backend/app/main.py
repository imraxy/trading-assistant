...
from .api.routes import llm_debug as llm_debug_routes
...
try:
    ...
    # Load backend/.env first; do not override existing process env
    load_dotenv(_env_path_backend, override=False)
    _env_path_app = os.path.abspath(os.path.join(_here, ".env"))
    # Load app/.env too; still do not override process env or previous values
...
app.include_router(llm_debug_routes.router, prefix="/api/v1", tags=["LLM Debug"])
...
async def get_config():
    ...
    # Legacy provider model discovery; keep for backward compatibility
    _models = await get_models_map(prefer_live=True)
    _defaults = {prov: (mods[0] if mods else None) for prov, mods in _models.items()}

    # Aggregator catalog curated shortlist (balanced defaults)
    try:
        curated = await model_catalog.get_top_models({
            "budget_usd_per_m": float(os.getenv("ROUTER_BUDGET_CEILING_USD_PER_M", "5") or 5),
            "latency_target_ms": int(os.getenv("ROUTER_LATENCY_TARGET_MS", "2000") or 2000),
        })
    except Exception:
        curated = {}
    ...
    return {
        ...
        "data": {
            ...
            "features": {
                ...
                "aggregators_enabled": os.getenv("AGGREGATORS_ENABLED", "true"),
            },
            "llm_providers": available_providers(),
            "llm_models": _models,  # legacy latest from providers
            "llm_defaults": _defaults,
            "model_catalog": {
                "curated": curated,
                "refresh_interval_hours": int(os.getenv("CATALOG_REFRESH_INTERVAL_HOURS", "24") or 24),
            },
        }
    }
